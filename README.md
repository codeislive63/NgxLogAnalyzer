...
- **upstream** https://gitlab.education.tbank.ru/backend-academy-dotnet-2025/homeworks/3_logs
- **fork** `https://gitlab.education.tbank.ru/backend-academy-dotnet-2025/homeworks-forks/<student>/3_logs`
...
```shell
dotnet run --project ./src/LogsApp/Logs.csproj --path logs/*.log --format markdown --output report.md --from 2025-03-01
```
...
- Программа должна логировать все важные события, которые происходят во время ее работы.
    ...
    - Для логирования необходимо использовать `Serilog` (НЕ подключен в шаблоне).
...
Общее кол-во баллов за работу = 120 б. + 25 доп. б. = **125 б.**
