...
runTest "negative" "input file does not exist" 2 \
  -p /tmp/data/input/nonexistent.txt -f json -o ${pwd}/scripts/data/output/output1.json

runTest "negative" "input file has unsupported extension" 2 \
  -p /tmp/data/input/file1.log -f json -o ${pwd}/scripts/data/output/output2.json

runTest "negative" "output file already exists" 2 \
  -p /tmp/data/input/file2.txt -f json -o ${pwd}/scripts/data/output/existing.json

runTest "negative" "output file has unsupported extension (JSON)" 2 \
  -p /tmp/data/input/file2.txt -f json -o ${pwd}/scripts/data/output/output4.txt

runTest "negative" "output file has unsupported extension (MD)" 2 \
  -p /tmp/data/input/file2.txt -f markdown -o ${pwd}/scripts/data/output/output5.txt

runTest "negative" "output file has unsupported extension (AD)" 2 \
  -p /tmp/data/input/file2.txt -f adoc -o ${pwd}/scripts/data/output/output6.txt

runTest "negative" "unsupported output format" 2 \
  -p /tmp/data/input/file2.txt -f txt -o ${pwd}/scripts/data/output/output7.txt

runTest "negative" "invalid date format (--from)" 2 \
  -p /tmp/data/input/file2.txt -f txt -o ${pwd}/scripts/data/output/output8.json --from="2025.01.02"

runTest "negative" "invalid date format (--to)" 2 \
  -p /tmp/data/input/file2.txt -f txt -o ${pwd}/scripts/data/output/output9.json --to="2025.01.02"

runTest "negative" "--from > --to" 2 \
  -p /tmp/data/input/file2.txt -f txt -o ${pwd}/scripts/data/output/output10.json --from="2025-01-02" --to="2025-01-01"
...
runTest "negative" "required parameter -f is missing" 2 \
  -p /tmp/data/input/nonexistent.txt -o ${pwd}/scripts/data/output/output12.json

runTest "negative" "required parameter -o is missing" 2 \
  -p /tmp/data/input/nonexistent.txt -f json -o ${pwd}/scripts/data/output/output13.json

runTest "negative" "unsupported parameter is present" 2 \
  -p /tmp/data/input/nonexistent.txt -f json -o ${pwd}/scripts/data/output/output14.json --custom=argument

runTest "positive" "properly calculate statistics from multiple local files" 0 \
  -p /tmp/data/input/logs**.txt -f json -o ${pwd}/scripts/data/output/stats.json
